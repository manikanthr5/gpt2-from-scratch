...
class GPTConfig:
    block_size: int = 1024
    vocab_size: int = (
        50257  # number of tokens: 50,000 BPE merges + 256 bytes tokens + 1 <EOS token>
    )
    n_layer: int = 12
    n_head: int = 12
    n_embed: int = 768
...
class GPT(nn.Module):
    ...
    def forward(self, idx):
        ...
        ), f"Cannot forward of length {T}, model block size is exhausted."
        # forward the token and position embeddings
        pos = torch.arange(0, T, dtype=torch.long, device=idx.device)  # shape (T)
        pos_emb = self.transformer.wpe(pos)  # position embeddings of shape (T, n_embed)
        tok_emb = self.transformer.wte(idx)  # token embeddings of shape (B, T, n_embed)
        x = tok_emb + pos_emb  # pos_emb get broad cased to shape (B, T, n_embed)
        # forward the blocks of the transformer
        for block in self.transformer.h:
            x = block(x)
        # forward the final layer norm and the classifier
        x = self.transformer.ln_f(x)
        logits = self.lm_head(x)  # shape of (B, T, vocab_size)
        return logits
    ...
    def from_pretrained(cls, model_type):
        ...
        print("Loading pretrained model from huggingface gpt: %s" % model_type)
...
for i in range(num_return_sequences):
    ...
    print(">", decoded)  # print the decoded text
